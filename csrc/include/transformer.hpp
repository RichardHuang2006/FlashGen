...
struct LayerWeights {
    ...
    float* W1         = nullptr;   // [d_ff, d_model]  up-stream (GPT-2 GELU / LLaMA SwiGLU up)
    float* W1_gate    = nullptr;   // [d_ff, d_model]  gate-stream for SwiGLU (LLaMA only; nullptr for GPT-2)
...
public:
    ...
    /// Load weights from a flat binary file (legacy path).
    void load_weights(const std::string& path, cudaStream_t stream);

    /// Set weights from externally owned device pointers (Python → C++ bridge).
    /// The caller (Python side) is responsible for keeping the source tensors
    /// alive for the entire lifetime of this Transformer object.
    void set_weights(const TransformerWeights& w) { weights_ = w; }
