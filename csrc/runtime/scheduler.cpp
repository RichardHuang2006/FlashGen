...
void Scheduler::try_resume_preempted(SchedulerOutput& output, int& token_budget) {
    while (!preempted_.empty()) {
        ...
        if (!can_allocate_blocks(blocks_needed) || tokens_needed > token_budget) {
...
void Scheduler::post_step(const std::vector<SequenceGroup*>& batch,
    ...
    // Move finished sequences out of running queue; keep shared_ptrs alive
    // so the caller can safely access them after removal.
    for (auto it = running_.begin(); it != running_.end(); ) {
