...
void InferenceEngine::step() {
    ...
    // 5. Update scheduler state; collect finished sequences (kept alive via
    //    shared_ptr so they remain valid through deliver_responses below).
    std::vector<std::shared_ptr<SequenceGroup>> finished_refs;
    scheduler_->post_step(batch, new_tokens, finished_refs);

    // 6. Deliver finished responses
    std::vector<SequenceGroup*> finished;
    finished.reserve(finished_refs.size());
    for (auto& sg : finished_refs) {
        finished.push_back(sg.get());
    }
